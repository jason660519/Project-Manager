...
| API Key 儲存 | `.env` 環境變數（server side）| Release：macOS Keychain；`tauri dev`：預設 `~/.project-manager/dev-secrets.json`（免 Keychain 彈窗） |
...
> Port 43187 被其他 process 佔用時，可用 `DEVPILOT_FORCE_KILL_PORT=1 ./start_project_manager.sh start` 強制接管。
