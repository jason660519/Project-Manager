...
| `spawnAgent(opts)` | `spawn_agent` | Calls `/api/bridge/execute` dry run | Returns `{ pid, spawnToken }`. `spawnToken` (unique, monotonic) is the event correlation key; `pid` is for `killProcess` / display. Browser mode returns `{ pid: 0, spawnToken: 0 }`. See ADR-014. |
...
| --- | --- | --- | --- |
| `agent-stdout` | `{ pid, spawnToken, line }` | `spawn_agent` stdout task | Runs, logs, sessions UI |
| `agent-stderr` | `{ pid, spawnToken, line }` | `spawn_agent` stderr task | Runs, logs, sessions UI |
| `agent-exit` | `{ pid, spawnToken, code }` | process wait task | Run completion state |
...
**Correlation contract:** consumers MUST correlate events to a run by `spawnToken` (unique, monotonic), never by `pid`. OS PIDs are reused, so PID-based correlation can replay a stale exit onto an unrelated run; the token closes that race. See ADR-014.
...
| `spawnAgent(opts)` | `spawn_agent` | 呼叫 `/api/bridge/execute` dry run | 回傳 `{ pid, spawnToken }`。`spawnToken`（唯一、單調遞增）是事件對應鍵；`pid` 供 `killProcess` 與顯示用。Browser mode 回傳 `{ pid: 0, spawnToken: 0 }`。見 ADR-014。 |
...
**對應契約：** 消費者必須以 `spawnToken`（唯一、單調遞增）對應事件到某次執行，絕不可用 `pid`。OS PID 會被重用，PID-based 對應可能把過時的 exit 重播到無關的執行；token 關閉此 race。見 ADR-014。
