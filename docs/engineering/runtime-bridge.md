...
DevPilot has two runtime modes:
...
| `scanProjects(root)` | `scan_projects` | Throws | Finds direct child folders containing `.dev-pilot.json`. |
...
`spawn_agent` currently accepts command, args, and working directory from the renderer wrapper. Before broader beta usage, DevPilot needs a formal allowlist and approval policy. Until then:
...
| `scanProjects(root)` | `scan_projects` | Throws | 找出直接子資料夾內的 `.dev-pilot.json`。 |
