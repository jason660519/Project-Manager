...
| --- | --- | --- | --- | --- |
| Provider API keys | OS Keychain via `set_secret` / `get_secret` | `~/.project-manager/dev-secrets.json` (default) | `localStorage` | Debug file avoids macOS Keychain prompts on unsigned rebuilds. Override with `PM_DEV_PLAINTEXT_SECRETS`. |
...
**Engineer notice:** see [`.project-manager/dev-logs/dev-keychain-bypass-2026-05-20.md`](../../.project-manager/dev-logs/dev-keychain-bypass-2026-05-20.md) for migration steps (re-save keys once after switching backends). Keys UI and Settings show the active backend via `secrets_storage_backend`.
...
| --- | --- | --- | --- | --- |
| Provider API keys | OS Keychain（`set_secret` / `get_secret`） | 預設 `~/.project-manager/dev-secrets.json` | `localStorage` | Debug 檔避免 unsigned rebuild 反覆跳出 Keychain。可用 `PM_DEV_PLAINTEXT_SECRETS` 覆寫。 |
...
**工程師提醒：** 見 [`.project-manager/dev-logs/dev-keychain-bypass-2026-05-20.md`](../../.project-manager/dev-logs/dev-keychain-bypass-2026-05-20.md)（切換 backend 後請到 Keys 重新 Save 一次）。Keys / Settings 會透過 `secrets_storage_backend` 顯示目前 backend。
