...
The ingestion pipeline converts loose source material into reviewed `Feature` drafts before writing to `.dev-pilot.json`. User review is required before imported drafts become canonical project data.
...
4. Keep `.dev-pilot.json` as the canonical save target.
...
Ingestion pipeline 將鬆散來源資料轉成待審核的 `Feature` drafts，再寫入 `.dev-pilot.json`。Imported drafts 成為正式 project data 前必須經過使用者 review。
...
4. `.dev-pilot.json` 是 canonical save target。
