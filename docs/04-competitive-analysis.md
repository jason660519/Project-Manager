...
DevPilot sits at the intersection of project management, AI IDE tooling, and desktop launchers. The core white space is local-first, agent-native task orchestration.
...
- DevPilot is complementary as an execution layer.
...
- DevPilot targets lightweight solo/small-team execution flow.
...
Conclusion: DevPilot solves engineering-specific dispatch pain more directly.
...
Conclusion: coexistence is likely; DevPilot can borrow overlay interaction patterns.
...
Conclusion: DevPilot acts as planning/dispatch/monitoring layer above IDE assistants.
...
Conclusion: integrate GitHub issues as feature sources for DevPilot.
...
2. Progress as Code (`.dev-pilot.json`).
3. Local Rust runtime for process/FS/OS operations.
4. AI-assisted spec ingestion pipeline.

One-line position:

DevPilot connects planning intent, agent execution, and runtime visibility in one local workflow.

## 6. Capability Comparison

| Capability | DevPilot | Linear | Notion | Raycast | Claude Code |
| --- | --- | --- | --- | --- | --- |
...
DevPilot 位於專案管理、AI IDE、桌面啟動器三個市場交會處，主要空白是「本機優先 + Agent 原生 + 任務指揮」。
...
- 通用性高，但工程派遣流程需手動拼裝。
- DevPilot 直接解工程任務派遣與監控痛點。

結論：DevPilot 在工程執行場景更直接。
...
結論：可共存，DevPilot 可借鑑 overlay 互動模式。
...
結論：DevPilot 應作為 IDE 之上的規劃、派遣與監控層。
...
結論：可把 GitHub issue 納入 DevPilot 的 Feature 來源。
...
DevPilot 把規劃意圖、Agent 執行與即時可見性，整合成單一本機工作流。

## 6. 功能比較

| 能力 | DevPilot | Linear | Notion | Raycast | Claude Code |
| --- | --- | --- | --- | --- | --- |
| 本機優先 | 是 | 否 | 否 | 部分 | 是 |
