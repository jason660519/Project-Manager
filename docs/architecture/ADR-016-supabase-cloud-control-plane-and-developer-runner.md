# ADR-016: Supabase Cloud Control Plane and Developer Runner
...
Use Supabase as Project Manager's cloud control plane:

- Supabase Auth owns identity and session lifecycle.
- Supabase Postgres owns collaborative product state.
- Supabase Row Level Security or equivalent backend-enforced checks protect workspace-scoped rows.
- Supabase Storage and/or Realtime may be used for report assets, large execution artifacts, and live state when their access model is defined.
...
Supabase gives the product the missing cloud primitives needed for Developer/User login:
...
```text
Supabase = cloud brain and collaboration state
...
Rejected as the default path. It is too heavy for general Users and makes cloud portal, team collaboration, device changes, and workspace permissions harder. It remains viable as a future self-hosted or enterprise deployment mode.
...
| --- | --- | --- |
| Auth sessions | Supabase Auth | Do not create custom password storage. |
| Users/workspaces/memberships/roles | Supabase Postgres | Protected by RLS or backend policy checks. |
| Project metadata and solution URLs | Supabase Postgres | Supports User Portal and reports. |
| Feature/task state | Supabase Postgres during cloud migration | Define sync precedence with project files before writeback. |
| Feature specs, TDD specs, dev logs | Git/project files | Cloud stores metadata and links. |
| Run metadata | Supabase Postgres | Status, runner ID, duration, result summary. |
...
- Self-hosted mode can be designed later as Docker Compose with API, Postgres, storage, and runner components; it should not be the default MVP path.
