...
| 防回歸檢查 | 舊入口 / 舊 sheets 是否被擋下 | dev log 的「驗證結果」段落；navigation、dashboard sheets、xmux 相關工作必填 `npm run guard:legacy-surfaces` |
...
- 報告內容**必須**包含 dashboard 會抓的欄位（見上表）：
  ...
  7. **防回歸檢查**（若動到 navigation、dashboard sheets、xmux/browser，列出 `npm run guard:legacy-surfaces` 與必要的 `npm run branch:check` 結果）
