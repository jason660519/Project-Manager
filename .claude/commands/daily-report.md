...
| 防回歸檢查 | 舊入口 / 舊 sheets 是否被擋下 | dev log 的「驗證結果」段落；navigation、dashboard sheets、cmux 相關工作必填 `npm run guard:legacy-surfaces` |

任何**缺少 feature ID 對齊**的 dev log 在 dashboard 上會變成「孤兒」，無法被自動 reflect。**寫日誌時請先決定 affected feature IDs。**

## Artifact 標準格式

每個 feature folder（`.project-manager/features/<ID>/`）包含標準文件。建立新 feature 或更新既有 artifact 時，遵循以下格式。
...
高成本 debug、回歸修復、或使用者重複回報的缺陷必須建立或更新此檔。它是後續 TDD/E2E 場景設計的經驗來源，不是一般流水帳。

必須包含以下段落：

| 段落 | 說明 |
|---|---|
| `## Summary` | 一段話描述本次缺陷與修復範圍 |
| `## User-Reported Symptoms` | 表格列出使用者看到的症狀、截圖/描述證據 |
| `## Reproduction Paths` | 可手動重現的步驟；若無法重現，寫明限制 |
| `## Root Cause` | 根因與錯誤假設，避免只寫「改了哪些檔案」 |
| `## Final Fix` | 最終修復策略與保留的防護行為 |
| `## Tests Added Or Updated` | 單元、整合、E2E、手動 smoke 對應測試 |
| `## Verification Evidence` | 指令與結果，不能填未執行的命令 |
| `## Lessons For Future TDD / E2E` | 可複用的測試設計經驗 |
| `## Follow-Up Candidates` | 尚未完成但值得升級的測試或工具化項目 |
...
此檔將真實使用者操作路徑轉成 TDD 與 E2E 測試候選。每次 debug 發現新使用情境或邊界條件，都應 append 或更新此檔。

必須包含以下段落：

| 段落 | 說明 |
|---|---|
| `## Purpose` | 說明此 scenario map 對應哪個 feature/bugfix |
| `## Scenario Matrix` | 表格：Scenario ID、User Path、Risk、Unit/Integration Coverage、E2E Candidate、Status、Source |
| `## Unit Test Backlog` | 需要補到 unit/integration 的邊界用例 |
| `## E2E Candidate Backlog` | 需要升級為 E2E 的核心流程 |
| `## Conversion Rule` | 後續 debug 如何把新情境加入測試庫 |
...
- 報告內容**必須**包含 dashboard 會抓的欄位（見上表）：
  ...
  7. **防回歸檢查**（若動到 navigation、dashboard sheets、cmux/browser，列出 `npm run guard:legacy-surfaces` 與必要的 `npm run branch:check` 結果）
...
| `paths.spec` / `tdd` / `debugRetro` / `testScenarios` / `implementation` | 對應交付物路徑；標準檔名優先用 `feature-spec.md` / `tdd-spec.md` / `debug-retro.md` / `test-scenarios.md` |
