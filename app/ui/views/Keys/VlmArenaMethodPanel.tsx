  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-12">
      <div className="border border-stone-200/18 bg-[rgb(var(--pm-panel))]/72 p-4 shadow-xl backdrop-blur-sm rounded-sm lg:col-span-4">
        ...
        <div className="mt-3">
          <label className="mb-1 block text-[10px] uppercase tracking-[0.14em] text-stone-400">{copy.imageDetail}</label>
          <select
            value={imageDetail}
          ...
          >
            ...
            <option value="low">{copy.imageDetailLow}</option>
            <option value="high">{copy.imageDetailHigh}</option>
          </select>
        </div>
      </div>
        ...
        <div className="overflow-x-auto border border-stone-200/12 bg-black/10">
          <table className="min-w-full border-collapse text-left">
            <thead className="sticky top-0 z-10 border-b border-stone-200/12 bg-[rgb(var(--pm-panel))]">
              <tr>
                <th className="px-3 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-stone-400">Dimension</th>
                <th className="px-3 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-stone-400">Observation</th>
                <th className="px-3 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-stone-400">Pass Rule</th>
              </tr>
            </thead>
            <tbody>
              {copy.methodRows.map((row) => (
