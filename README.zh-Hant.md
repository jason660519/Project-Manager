...
[**📖 專案公開文件網站 (GitHub Pages)**](https://jason660519.github.io/Project-Manager/) | [English README](./README.md)

Project Manager 是一個跨專案、跨工種、跨專業、跨語言、跨工程領域的 project-management orchestration app。它會從 Folder、GitHub 與各類 domain documents 攝取專案規格與工作輸入，AI-normalize 成 canonical project data，再協助 PM 與各領域專家跨 human roles、AI agents、tools 與 review queues 進行 triage、planning、dispatch 與 reporting。

軟體專案、IDE 與 coding agents 只是其中一個 supported vertical，不是產品邊界；同一套 orchestration model 應可服務 software、product、architecture、construction、structural、civil、MEP、procurement、operations、QA 與 executive stakeholders。

## 專案路徑

- **代碼主體**: `<你的本機 Project-Manager 路徑>`
- **App Router**: `app/`
- **核心類型**: `lib/types/`
- **適配器**: `lib/adapters/`
...
3. **適配器架構 (Adapter Pattern)**: 自由切換 IDE (Cursor/Trae)、agent CLI (Claude Code/Codex)、domain tools 或未來 service integrations 等 execution surfaces。
4. **上下文感知 (Context-Aware)**: 自動從使用者專案內容或現有規格書中提取並儲存上下文，為接下來的 AI Agent 提供精準的 Prompt。
...
> **Browser mode 適合對象**：在自己機器上跑 `next dev` / `next start`，用瀏覽器當 UI 的開發者、PM power user 或領域專家。不適合部署到遠端 server 給多人使用（spawn 會在 server 機器上執行，不是使用者機器）。
...
- 透過 `/api/bridge/execute` 產生 adapter dry-run 執行計畫，涵蓋 IDE、agent 與未來 domain-tool targets。
...
- **非商業 / 個人使用**：完全免費且無限制。
- **商業使用**：僅允許個人或 3 位（含）以下使用者的團隊使用。超過 3 位使用者的商業團隊或將本軟體作為託管服務（SaaS）銷售，需購買商業授權。
