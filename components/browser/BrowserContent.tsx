...
export function BrowserContent({
  ...
  const [browserSessionKind, setBrowserSessionKind] = useState(() => sessionKind(itemId));
  const nativeActive = browserSessionKind === 'tauri';
  const selectElementAvailable = nativeActive || browserSessionKind === 'iframe';

  useEffect(() => {
    let cancelled = false;
    const sync = () => {
      if (cancelled) return;
      setBrowserSessionKind(sessionKind(itemId));
    };
    sync();
  ...
  const exitSelectElementMode = useCallback(
    (message?: string) => {
      ...
      setSelectMode(false);
      void cancelBrowserElementSelection(itemId).catch(() => {});
      if (message) {
  ...
  const completeSelectElement = useCallback(
    async (rawPayload: string, session: number) => {
      if (selectSessionRef.current !== session) return false;
      ...
      selectCapturedSinceStartRef.current = true;
      ...
      setStatus({
        tone: 'success',
        message: `Selected element captured (${tag}), sent to AI Assistant, and ready to drag. Select Element mode remains active.`,
      });
  ...
  const startSelectElement = useCallback(() => {
    ...
    selectCapturedSinceStartRef.current = false;
    setSelectMode(true);
    setSelectedDragContext(null);
    setStatus({
      tone: 'info',
    ...
    const runSelection = (activeSession: number) => {
      void selectBrowserElement(itemId)
        .then(async (rawPayload) => {
          const keepSelecting = await completeSelectElement(rawPayload, activeSession);
          if (keepSelecting && selectSessionRef.current === activeSession) {
            window.setTimeout(() => {
              if (selectSessionRef.current === activeSession) {
                runSelection(activeSession);
              }
            }, 80);
          }
        })
        .catch((err) => {
          if (selectSessionRef.current !== activeSession) return;
          const message = err instanceof Error ? err.message : String(err);
          if (selectCapturedSinceStartRef.current) {
            setSelectMode(true);
            setStatus({
              tone: 'warning',
              message: `Selected element was captured, but Select Element could not re-arm automatically: ${message}`,
            });
          } else {
            setSelectMode(false);
    ...
    runSelection(session);
  }, [completeSelectElement, exitSelectElementMode, itemId, nativeUnavailable, selectElementAvailable, selectMode]);
