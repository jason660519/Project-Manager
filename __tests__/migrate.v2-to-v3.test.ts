...
describe('migrateConfig v2 → v3', () => {
  it('exposes 3 as the current schema version', () => {
    expect(CURRENT_SCHEMA_VERSION).toBe(3);
  });

  it('bumps schemaVersion to 3 on a v2 document', () => {
    const v2 = {
      schemaVersion: 2,
      id: 'aaa',
  ...
  it('defaults every feature to phase=development and points=1 when missing', () => {
    const out = migrateConfig({
      ...
      features: [
        ...
        { id: 'F02', name: 'f', category: 'c', status: 'todo', progress: 0, paths: {}, phase: 'testing' },
    ...
    expect(out.features[1].phase).toBe('testing');     // existing value preserved
  ...
  it('chains v1 → v2 → v3: a fresh v1 document still ends with phase + points populated', () => {
    const out = migrateConfig({
  ...
  it('is idempotent — running a v3 document through the pipeline does not change it', () => {
